...
const content = fs.readFileSync(erlicPath, 'utf-8');
        ...
        const ranked = help
            ...
            .filter(v => v.sim >= 0.5 && v.sim <= 0.9)
            .sort((a, b) => b.sim - a.sim);

        if (ranked.length && /^[^a-zA-Z0-9]/.test(budy) && !m.fromMe && !ranked.some(item => item.cmd === command.toLowerCase())) {
            m.reply(`Command tidak ditemukan, mungkin maksud kamu:\n\n${ranked.map(v => `➠ *${pripek}${v.cmd}* (${(v.sim*100).toFixed(1)}%)`).join('\n')}`);
        }
    } catch (e) {
        console.error(e);
    }
...
func.addSaldo = (userId, amount) => {
    let position = null
    Object.keys(global.db.saldo).forEach((x) => {
        if (global.db.saldo[x].id === userId) {
            position = x
        }
    })
    if (position !== null) {
        global.db.saldo[position].saldo += amount
    } else {
        let obj = ({
            id: userId,
            saldo: amount
        })
        global.db.saldo.push(obj)
    }
}

func.minSaldo = (userId, amount) => {
    let position = null
    Object.keys(global.db.saldo).forEach((x) => {
        if (global.db.saldo[x].id === userId) {
            position = x
...
func.cekSaldo = (userId) => {
    ...
    if (position !== null) {
        return global.db.saldo[position].saldo
    } else {
        return 0
    }
}
...
func.logFile = (log, filename = 'bot') => {
    ...
    const logfile = fs.createWriteStream(path.join(process.cwd(), `${filename}.log`), {
        flags: 'a'
    })
...
func.checkError = (error) => {
    ...
    if (err.includes('rate-overlimit')) status = true;
    else if (err.includes('internal-server-error')) status = true;
    else if (err.includes('this.isZero')) status = true;
    else if (err.includes('Connection Closed')) status = true;
    else if (err.includes('Timed Out')) status = true;
    else if (err.includes('Value not found')) status = true;
    else if (err.includes('service-unavailable')) status = true;
    else if (err.includes('Maximum call stack size exceeded')) status = true;
...
func.messageId = function(length) {
    return 'ERLIC' + crypto.randomBytes(length).toString('hex').toUpperCase()
}
...
func.formatDuration = function(duration) {
    let result = '';
    if (duration) {
        if (duration.endsWith('y')) result = duration.replace(/y/gi, ' years');
...
func.createClue = function(word = '') {
    let clue = '';
    for (let i = 0; i < word.length; i++) {
        if (i % 2 === 0) { // Menampilkan huruf pada posisi genap
            clue += word[i];
        } else {
            clue += '-'; // Menyembunyikan huruf pada posisi ganjil
        }
    }
    return func.texted('monospace', clue);
}
...
func.timezone = function(timeZone) {
    if (!['Asia/Jakarta', 'Asia/Makassar', 'Asia/Jayapura'].includes(timeZone)) timeZone = 'Asia/Jakarta';
    ...
    let zoneLabel = 'WIB'; // Default ke WIB
...
func.fpayment = {
    ...
    message: {
        requestPaymentMessage: {
            ...
            noteMessage: {
                extendedTextMessage: {
                    text: 'Copyright © 2024 SuryaDev, AI. Erlic-Bot'
                }
            },
...
func.fverified = {
    ...
    message: {
        conversation: 'Erlic Verified By WhatsApp'
    }
}
...
func.fetchBuffer = async (file, options = {}) => {
...
func.obfus = async function(query, type = 'low') {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            const obfuscationResult = jsobfus.obfuscate(query, method[type]);
            const result = {
                status: 200,
                developer: 'SuryaDev.',
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const {
                data,
                status
            } = await axios('https://getmyfb.com/process', {
                method: 'POST',
...
func.tmpfiles = async function(buffer) {
    const {
        ext,
        mime
    } = (await fromBuffer(buffer)) || {};
